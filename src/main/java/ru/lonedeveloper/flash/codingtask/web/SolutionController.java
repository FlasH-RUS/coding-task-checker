...
public class SolutionController {
	...
	@Autowired
	public SolutionController(final SolutionRepository solutionRepository) {
		this.solutionRepository = solutionRepository;
	}

	@RequestMapping(method = RequestMethod.POST, produces = "text/html")
	public String create(@Valid final Solution solution, final BindingResult bindingResult, final Model uiModel,
	...
	private boolean isTaskAlreadySolved(final HttpServletRequest request, final Solution newSolution) {
		return solutionRepository.countByIpAndTaskAndSuccessful(request.getRemoteAddr(), newSolution.getTask(),
				true) > 0;
	}
	...
	public String list(@RequestParam(value = "sortFieldName", required = false) final String sortFieldName,
			@RequestParam(value = "sortOrder", required = false) final String sortOrder, final Model uiModel,
